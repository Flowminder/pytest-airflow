...
3 below. The DAG begins with the `__pytest_source` task which is an
...
used to generate the set of all desired tests. `__pytest_source` is
...
`__pytest_source` would mark all tasks that do not have the keyword `foo`
...
subgraph PythonOperator
    __pytest_source --> testfoo[tests..test_foo.py-test_foo_does_something]
    __pytest_source --> test...[...]
    __pytest_source --> testbar[tests..test_bar.py-test_bar_is_true]
end

subgraph PythonOperator
    testfoo --> __pytest_sink
    test... --> __pytest_sink
    testbar --> __pytest_sink
end
```
_Figure 3: pytest-airflow DAG_

Directly, connected downstream to `__pytest_source` are all the test
...
Finally, the last task in the DAG is `__pytest_sink` which is an
...
cmdline interface. First it adds four new options to `pytest` using the
...
id. If not explicitly defined, the DAG is named `pytest` by default. The last
two options are `--source` and `--sink` that determines the name of the source
and sink tasks as discussed in the previous section.

The second hook implemented in this section is `pytest_cmdline_main`.
`pytest-airflow` wraps the execution of the call to `pytest_cmdline_main` with
the intention of adding a pointer to the DAG generated by the plugin to
`config._dag` and forcing the program loop to return this pointer, as well as
...
- `dag_report`: the callable for `__pytest_sink`. Its argument should be
...
`pytest` collects fixtures with the fixture manager starting with the
broadest ones defined in plugins to the narrowest ones defined in the test
files themselves. The order which the manager collects fixtures defined in
plugins is not deterministic. Therefore, before looping through all the
fixtures, we place the default fixtures defined in this plugin at the top
of the list such that they function as fallbacks in case the user did not
specify the requested fixtures in any other place. This operation comes in
hand when the user requests the use of a custom DAG outside of the test
path in order to integrate with a pre-existing DAG. In this case, the DAG
must be passed with a custom plugin defined in the same file as the DAG as
...
With pointers to the required `FixtureDef` acquired, `_init_dag` proceeds
with computing the value of those fixtures by calling
`_compute_fixture_value`. In order to compute the value of a `FixtureDef`,
`pytest` requires a context. This context is provided by any test item in
the collection of items, thus we pick the first test item's context to
compute the value of the fixture. Test items store this context in
a pointer called `_request`. Once fixture values are calculated they are
stored in the `FixtureDef` itself in `cached_result`. We retrieve this
value and store them in `_request` in order to make them available to the
computation of other fixtures within that context. Finally, we return the
computed value.
...
`__pytest_source` and `__pytest_sink`. The `__pytest_source` is defined
...
callable to `__pytest_sink` is obtained by calling `_get_fixture` with
...
`__pytest_source` and downstream to `__pytest_sink`.
...
passed to the `xcom` channel making them available to `__pytest_sink`.
